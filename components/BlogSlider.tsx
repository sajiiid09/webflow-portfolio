    ...
    <section id="blog">
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-slate-300">Tech Blogs</p>
            <h2 className="mt-4 text-3xl font-semibold md:text-4xl text-slate-50">Latest writing & research notes.</h2>
          </div>
        ...
        <div ref={sliderRef} className="mt-10 flex gap-6 overflow-x-auto scroll-smooth snap-x snap-mandatory pb-4">
          {blogPosts.map((post) => (
            <motion.article
              key={post.title}
              className="min-w-[280px] flex-1 snap-center rounded-3xl glass-surface glass-card p-6 text-slate-100 md:min-w-[360px]"
